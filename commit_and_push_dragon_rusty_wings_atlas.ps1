...
Write-Host "== PixelMania client: commit + push dragon_rusty_wings atlas migration ==" -ForegroundColor Cyan
...
Write-Host "This switches dragon_rusty_wings from standalone Assets/items/back_items/*.png files" -ForegroundColor Yellow
Write-Host "to the shared back_item.png atlas (row 2), same pattern as appreciation_wings/devil_wings." -ForegroundColor Yellow
...
$commitMessage = @"
Make dragon_rusty_wings fully atlas-driven (client)

- Add dragon_rusty_wings_icon + dragon_rusty_wings_1..5 to Data/items/wearable_atlas.json
  as explicit-atlas frame dictionaries pointing at Assets/items/back_item.png row 2: icon
  at atlas coord (0,2) -> region [0,64,32,32]; five 64x32 flap frames at atlas coords
  (1,2),(3,2),(5,2),(7,2),(9,2) -> regions [32,64,64,32], [96,64,64,32], [160,64,64,32],
  [224,64,64,32], [288,64,64,32]. Same pattern as appreciation_wings (row 0) and
  devil_wings (row 1).
- item_database.gd's dragon_rusty_wings entry now references those atlas keys instead of
  res://Assets/items/back_items/dragon_rusty_wings_*.png files: texture/inventory_icon and
  the idle_frames/jump_frames/fall_frames/flap_frames arrays. sprite_folder/idle_sprite
  dropped. Frame ordering unchanged from the old entry (idle=[1,2,3], jump=[2,4,1],
  fall=[3,5,1], flap=[2,4,1] -- same as appreciation_wings), so in-game animation timing
  is identical -- only the texture source changed.
