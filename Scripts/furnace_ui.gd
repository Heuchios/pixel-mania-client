...
var furnace_grid_pos = Vector2i.ZERO
var gem_label = null
var info_label = null
...
func setup(parent_world, ui_node):
	...
	recipes = StationRecipes.get_recipes("furnace")
...
func apply_station_card_style(card: Panel, can_make: bool):
	...
	if can_make:
		fill = Color(0.42, 0.22, 0.08, 0.56)
		border = Color(1.0, 0.62, 0.18, 0.88)
...
func setup_panel():
	...
	panel = ui_layer_ref.get_node_or_null("FurnacePanel")

	if panel == null:
		panel = Control.new()
		panel.name = "FurnacePanel"
	...
	close_button.pressed.connect(close_furnace)
	...
	section_label.text = "SMELTING RECIPES"
	...
	info_label.text = "Smelt refined materials and furnace-only outputs."
...
func create_recipe_card(recipe: Dictionary, card_position: Vector2, _card_index: int):
	...
	var card = Panel.new()
	card.name = "FurnaceCard_" + output_id
	...
	if can_make:
		ready_strip.color = Color(1.0, 0.52, 0.12, 0.82)
	else:
	...
	smelt_button.name = "SmeltButton"
	smelt_button.text = "SMELT" if can_make else "MISSING"
	smelt_button.position = Vector2(10, card.size.y - 42.0)
	smelt_button.size = Vector2(card.size.x - 20.0, 34)
	smelt_button.mouse_filter = Control.MOUSE_FILTER_STOP
	smelt_button.disabled = not can_make
	apply_station_arcade_button_style(smelt_button, true, false, 16)

	smelt_button.pressed.connect(smelt_recipe.bind(recipe))
	card.add_child(smelt_button)
...
func smelt_recipe(recipe: Dictionary):
	...
	if world != null and world.has_method("should_use_server_authoritative_world_actions") and bool(world.should_use_server_authoritative_world_actions()):
		if request_server_smelt(recipe):
			return
	...
	if world != null and world.has_method("show_notification"):
		world.show_notification("Smelted " + get_item_display_name(str(output["item_id"]), str(output["category"])) + ".")


func request_server_smelt(recipe: Dictionary) -> bool:
	...
	return bool(network.send_inventory_transaction_request({
		"action": "furnace_recipe",
		"station_id": "furnace",
		"recipe_id": str(recipe.get("id", "")),
		"station_x": furnace_grid_pos.x,
		"station_y": furnace_grid_pos.y,
		"world": world.current_world_name if world != null else ""
	}))


func handle_inventory_transaction_result(data: Dictionary) -> bool:
	if str(data.get("action", "")) != "furnace_recipe":
		return false

	if str(data.get("station_id", "furnace")) != "furnace":
		return false
	...
	if world != null and world.has_method("show_notification"):
		world.show_notification(str(data.get("message", "Smelting finished.")))
...
func open_furnace(grid_pos: Vector2i):
	furnace_grid_pos = grid_pos
...
func make_color_rect_transparent(_node):
	pass
