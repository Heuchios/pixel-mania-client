...
func simulate(delta: float, tick: int) -> void:
	...
	_prune_invalid_node_references()
	...
	for candidate in _callbacks.keys():
		if not _is_valid_node_reference(candidate):
			continue
...
func _prune_invalid_node_references() -> void:
	_prediction_enabled_nodes.prune_invalid_objects()
	_predicted_nodes.prune_invalid_objects()
	_input_graph.prune_invalid_objects()

	var stale_nodes: Array = []
	for candidate in _callbacks.keys():
		if not _is_valid_node_reference(candidate):
			stale_nodes.append(candidate)
	for candidate in _simulated_ticks.keys():
		if not _is_valid_node_reference(candidate) and not stale_nodes.has(candidate):
			stale_nodes.append(candidate)

	for candidate in stale_nodes:
		_erase_node_reference(candidate)

func _erase_node_reference(candidate: Variant) -> void:
	_callbacks.erase(candidate)
	_simulated_ticks.erase(candidate)
	_input_graph.erase(candidate)
	_prediction_enabled_nodes.erase(candidate)
	_predicted_nodes.erase(candidate)

func _is_valid_node_reference(candidate: Variant) -> bool:
	if candidate == null:
		return false
	if candidate is Object and not is_instance_valid(candidate):
		return false
	return candidate is Node
