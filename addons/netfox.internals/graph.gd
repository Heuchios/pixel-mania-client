...
func prune_invalid_objects() -> void:
	var stale_nodes: Array = []
	for node in _links_from.keys():
		if _is_invalid_object_reference(node):
			stale_nodes.append(node)
	for node in _links_to.keys():
		if _is_invalid_object_reference(node) and not stale_nodes.has(node):
			stale_nodes.append(node)
...
func _is_invalid_object_reference(value: Variant) -> bool:
	if value == null:
		return true
	if value is Object:
