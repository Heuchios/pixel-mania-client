...
func _run() -> void:
	...
	# Erasing on either side of a chunk boundary must clear the renderer cache too.
	for pos in [Vector2i(31, 31), Vector2i(32, 31), Vector2i(31, 32), Vector2i(32, 32)]:
		assert(renderer.set_item_atlas_cell(pos, source_id, Vector2i.ZERO))
		renderer.erase_block_cell(pos)
		renderer.reconcile_active_cells()
		assert(foreground_layer.get_cell_source_id(pos) == -1)
	var manager = block_manager_script.new()
	var body := StaticBody2D.new()
	body.collision_layer = 1
	var shape := CollisionShape2D.new()
	shape.shape = RectangleShape2D.new()
	body.add_child(shape)
	manager.retire_block_node_for_removal(body)
	assert(body.collision_layer == 0 and body.collision_mask == 0)
	assert(shape.disabled, "Confirmed removal must disable collision before deferred freeing")
	body.free()
	manager.free()

	mock_world.free()
	print("[block-break-renderer] cache, chunk boundaries, node collision and rejoin visual tests passed")
	quit(0)
