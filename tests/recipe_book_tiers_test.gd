...
func run() -> void:
	...
	assert(counts == {"splicing": 175, "crafting": 4, "furnace": 3}, str(counts))
	...
	print("Recipe book OK: 175 splicing, 4 crafting, 3 furnace; sheet tiers, icons, search, links, filters and reusable tier tabs")
	book.queue_free()
	inventory.free()
	world.free()
	await process_frame
