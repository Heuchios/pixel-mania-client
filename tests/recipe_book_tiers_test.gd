...
func run() -> void:
	...
	assert(counts == {"splicing": 182, "crafting": 6, "furnace": 0}, str(counts))
	...
	print("Recipe book OK: 182 splicing, 6 crafting, 0 furnace; sheet tiers, icons, search, links, filters and reusable tier tabs")
	book.queue_free()
	inventory.free()
	world.free()
	await process_frame
