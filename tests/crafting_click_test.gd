...
func run():
	...
	var ui := Control.new()
	root.add_child(ui)
	var crafting = load("res://tests/crafting_click_fixture.gd").new()
	...
	for down in [true, false]:
		...
		await process_frame
	assert(crafting.info_label.text == "Crafting...")
	crafting.handle_inventory_transaction_result({"action": "craft_recipe", "station_id": "crafting_station", "ok": false, "message": "Inventory is full."})
	assert(crafting.info_label.text == "Inventory is full.")
